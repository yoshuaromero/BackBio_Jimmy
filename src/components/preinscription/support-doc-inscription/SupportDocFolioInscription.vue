...
import GlobalDropZoneFiles from '@/components/preinscription/support-doc-inscription/GlobalDropZoneFiles.vue'
...
const dropFile = async id => {
  try {
    await axios.delete (`/v1/files/inscriptions-supporting-documents?id=${id}`, { responseType: 'blob' })
...
const handleCheck = async item => {
  if(item.folio == null || item.folio === '') {
    ...
    try {
      ...
      const { data } = await axios.put(`/v1/inscriptions-supporting-documents/supporting-documents-check/${item.id}`, requestBody)
      ...
      <template #cell(actions)="{ item }">
        <div v-if="item.hash !== null && item.hash !== ''">
          ...
          <VBtn
            ...
            variant="text"
            color="secondary"
            @click="dropFile(item.id, item.name)"
          />
        </div>
      </template>
