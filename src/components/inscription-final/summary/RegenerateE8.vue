...
const mounted = async () =>  {
  ...
  clear()
  try {
    const { data } = await axios.get(`v1/inscription-final/preview/${props.inscriptionId}`, { responseType: 'blob' })
...
const regenerate = async () =>  {
  ...
  Swal.fire({
    ...
    preConfirm: async password => {
      ...
      try {
        await axios.post(`v1/inscription-final/regenerate`, {
...
<template>
  ...
  >
    ...
    <VCard title="Previsualizar Formulario E8">
      <VDivider class="mt-2" />
      <VCardText 
        v-if="isLoading" 
