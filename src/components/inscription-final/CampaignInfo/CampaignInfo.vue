...
import DialogForm from '@/components/inscription-final/CampaignInfo/DialogForm.vue'
...
const { inscriptionId } = toRefs(props)
...
const getCampaignInfo = async () => {
  ...
  if (!can('get-e8', 'campaign-information')) {
    return Swal.fire({
  ...
  try {
    ...
    const { data } = await axios.get(`/v1/campaign/get-campaign-information-e8/${inscriptionId.value}`)
    ...
    const dataFormate = data.map(e => ({
      ...
      nameCounter: (`${e.firstNameCounter || ''} ${e.secondNameCounter || ''} ${e.firstSurnameCounter || ''} ${e.secondSurnameCounter || ''}`).trim(),
      action: e.id,
      ...e,
    }))
...
const handleSubmit = async value => {
  try {
    ...
    const requestBody = {
      "inscriptionId": value.form.value.inscriptionId || inscriptionId.value,
      ...
      "financialEntity": value.form.value.financialEntity === -1 ? null : value.form.value.financialEntity,
      "accountType": value.form.value.accountType === -1 ? null : value.form.value.accountType,
      ...
      "all": value.form.value.applyAll,
      "phase": 'E8',
    }

    if(value.edit.value) {
      await axios.put('/v1/campaign/campaign-information-e8/' + value.form.value.id, requestBody)
    } else {
      await axios.post('/v1/campaign/campaign-information-e8', requestBody)
    }
    ...
    <VCard>
      <VRow>
        ...
        <VCol class="v-col-sm-6 px-12 py-12 text-end">
          <VBtn
            v-if="can('post-e8', 'campaign-information')"
