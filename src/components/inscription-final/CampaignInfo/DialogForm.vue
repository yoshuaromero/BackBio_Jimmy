...
import imageLogo from '@images/pages/dialog-banner-lg.png'
...
const { t } = useI18n()
const { inscriptionId, data: infoCandidate, edit } = toRefs(props)
...
const getDataFinancialEntity = async () => {
  try {
    ...
    if (error.response && error.response.status !== 500) {
      ...
      await Swal.fire({
        icon: 'warning',
        text: message || t('validations.no_financial_institutions'),
...
const getDataAccountType = async () => {
  try {
    ...
    if (error.response && error.response.status !== 500) {
      ...
      await Swal.fire({
        icon: 'warning',
        text: message || t('validations.no_account_type'),
        target: document.getElementById('formCampaign'),
      })
    }
  }
...
const getDataCandidates = async () => {
  try {
    const { data } = await axios.get(`/v1/candidates/final-list/${inscriptionId.value}`)
    ...
    if (error.response && error.response.status != 500) {
      ...
      await Swal.fire({
        icon: 'warning',
        text: message || t('validations.no_candidate'),
        target: document.getElementById('formCampaign'),
      })
    }
  }
...
const accountTypeOptions = computed(() => {
  ...
  return [ { title: 'Seleccione un tipo de cuenta', value: -1 }, ...array]
})

const candidatesOptions = computed(() => {
  const array = dataCandidates.value
    .map(e => ({ title: e.nameCandidates, value: e.id }))
...
const handlerSubmit = async () => {
  ...
  if(form.value.firstNameManager === '') {
    await Swal.fire({
      icon: 'warning',
      text: t('validations.data_manager'),
      target: document.getElementById('formCampaign'),
    })
  ...
  if((form.value.dniCounter != null && form.value.dniCounter.length > 0) && (form.value.firstNameCounter === null || form.value.firstNameCounter.length === 0)) {
    await Swal.fire({
      icon: 'warning',
      text: t('validations.data_counter'),
      target: document.getElementById('formCampaign'),
    })
  ...
  if (form.value.candidateDni === null || form.value.candidateDni === "") {
    const idCandidate = dataCandidates.value.find(e => e.id === form.value.idCandidat)
...
const searchAni = async dni => {
  try {
    ...
    if (error.response && error.response.status != 500) {
      ...
      await Swal.fire({
        icon: 'warning',
        text: message || t('validations.no_dni'),
        target: document.getElementById('formCampaign'),
      })
    }
...
onBeforeUpdate(() => {
  ...
  if(infoCandidate.value !== null) {
    ...
    infoCandidate.value.idCandidat = idCandidate?.id
      ...
      <VForm id="formCampaign" @submit.prevent="handlerSubmit">
        ...
        <VCardText>
          <VRow justify="space-between">
            ...
            <VCol class="v-col-sm-4">
              <VCheckbox
                v-model="form.applyAll"
                :label="t('preinscription.grouping_political.apply_all')"
              />
            </VCol>
          </VRow>
          <VRow>
