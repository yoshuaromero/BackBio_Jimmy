...
const lock = computed(() => props.hasChangeSameE7 || props.hasChangeOtherE7)
...
<template>
  <VCard variant="outlined" class="pa-2 bg-deep-orange-darken-4">
    <template v-if="!readonly && !lock">
      <div v-if="can('create', 'candidates-changes')" class="tools">
    ...
    <VRow v-if="lock">
      <VCol 
        ...
        class="text-center 
          ...
          border border-primary mt-3 
          bg-secondary"
      >
        <template v-if="hasChangeOtherE7">
          {{ t("candidates.change_other_e7") }}
        </template>
        <template v-else-if="hasChangeSameE7">
          {{ t("candidates.change_same_e7") }}
        </template>
      </VCol>
    </VRow>
  </VCard>
