...
const minAgeCandidate = computed(() => inscriptionModifyStore.minAgeCandidate)
const candidatesDni = computed(() => inscriptionModifyStore.candidates.list.map(e => e.dni))
const genders = computed(() => generalfilterStore.genders.map(e => ({ title: e.description, value: e.id })))
const reasonsRemoved = computed(() => generalfilterStore.reasonsRemoved.map(e => ({ title: e.name, value: e.id })))
const isCoalition = computed(() => inscriptionModifyStore.isCoalition)

const isDoubleMilitancy = ref(false)
const messageDoubleMilitancy = 'Candidato presenta doble militancia.'

const isDoubleRegistry = ref(false)
const messageDoubleRegistry = 'Candidato presenta doble inscripción.'
...
const handlerSubmit = async () => {
  ...
  const requestBody = {
    ...form.value,
    candidateFinalId: props.data.id,
    oldGroupingPoliticalId: props.data?.groupingPoliticalId,
    oldDni: props.data.dni,
    oldFullname: `${props.data.firstname} ${props.data.secondname} ${props.data.firstsurname} ${props.data.secondsurname}`,
  }

  inscriptionModifyStore
    .saveCandidate(requestBody)
    .then(() => {
      v$.value.$reset()  
      form.value = {}
      isDoubleRegistry.value = false
    ...
    <VCard>
      ...
      <VForm @submit.prevent="handlerSubmit">
        <div class="mt-4 text-center text-h6 font-weight-semibold">
          Sección Candidato Retirado
        </div>
        <VCardText>
          <VRow>
            <VCol cols="12" sm="6" md="2">
              <VTextField
                :model-value="data.rowField"
            ...
            <VCol cols="12" sm="6" md="5">
              <VSelect
                ...
                :label="`${t('candidates.form.reason_removed')} *`"
                :items="reasonsRemoved"
                :error-messages="v$.reasonId.$errors[0]?.$message"
              />
            </VCol>
            <VCol cols="12" md="5">
              <VTextField
                :model-value="data.dni"
            ...
            <VCol cols="12" md="6">
              <VTextField
                :model-value="`${data.firstname} ${data.secondname}`"
                :label="t('candidates.form.names')"
                class="readonly-control"
                readonly
              />
            </VCol>
            <VCol cols="12" md="6">
              <VTextField
                :model-value="`${data.firstsurname} ${data.secondsurname}`"
                :label="t('candidates.form.surnames')"
                class="readonly-control"
                readonly
              />
            </VCol>
