...
const radicate = computed(() => inscriptionModify.radicate)

const isLoading = ref(false)
const infoDoc = ref({ hasPhysicalDoc: false })

const canUpload = computed(() => !infoDoc.value.hasPhysicalDoc && can('update-physical-doc', 'inscription-change'))
const canDelete = computed(() => !!infoDoc.value.hasPhysicalDoc && can('update-physical-doc', 'inscription-change'))

const getInfoDoc = () => {
  axios
    .get(`/v1/inscription-change-approval/infodoc/${props.inscriptionId}/${radicate.value}`)
...
const download = async () =>  {
  ...
  try {
    const { data } = await axios.get(`v1/inscription-change-approval/download/${props.inscriptionId}/${radicate.value}/physical`, { responseType: 'blob' })
...
const handleDelete = async () =>  {
  Swal.fire({
    ...
    preConfirm: async () => {      
      try {
        await axios.delete(`v1/inscription-change-approval/physical-doc`, {
          params: {
  ...
  Swal.fire({
    ...
    preConfirm: async () => {
      try {
        ...
        await axios.put(`v1/inscription-change-approval/physical-doc`, formData, { 
