...
const handlerNext = async () => {
  ...
  axios.get(`/v1/candidates/report/candidate/definitive/export`, {
    params: payload,
    responseType: "blob", // importante
  }).then(({ data }) => {
    console.log(data)

    const nameFile = 'Generación Archivo Listado de Candidatos Definitivos.xlsx'
...
<template>
  <VCard :title="title">
    <VForm ref="form" validate-on="submit" @submit.prevent="handlerNext">
      <VCardText>
        <VRow>
          <h4>Generación Archivo Listado de Candidatos Definitivos</h4>
        </VRow>
        <br>
        <VRow class="mb-2">
          <VCol cols="12" md="6">
