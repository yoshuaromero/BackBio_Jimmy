...
const consultLogoup = async function(){  
  ...
  if (valid){
    axios.get("/v1/grouping-political-logo/" + props.groupingPoliticalId)
      .then(() => saveLogo())
  }
}
...
const mounted = () => {
  axios.get("/v1/files/grouping-political-logo/" + props.groupingPoliticalId)
    .then(response => {
      if (response.status == 200) 
          ...
          <VCol cols="12" class="d-flex align-center justify-center pa-6">
            <div class="position-relative">
              <div class="text-center">
                <TextSubtitle1 v-if="status === 0">
                  {{ messageComposable.get("MS0040") }}
                </TextSubtitle1>
                <TextSubtitle1 v-if="status === 1" class="text-success">
                  {{ messageComposable.get("MS0041") }}
                </TextSubtitle1>
                <TextSubtitle1 v-if="status === -1" class="text-error">
                  {{ messageComposable.get("MS0042") }}
                </TextSubtitle1>
              </div>

              <VRow v-if="logo" justify="center">
                <VCol cols="12" :sm="status === 1 ? 6 : 3" class="text-center">
