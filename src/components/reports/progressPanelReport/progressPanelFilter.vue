...
const addOptionDefaultRequired = array => {
...
const corporations = computed(() => {
  return addOptionDefaultRequired(corporationItems.value.map(e => ({ value: e.id, title: e.name })))
})
...
const departments = computed(() => {
  ...
  return addOptionDefaultRequired(array)
})
...
const rules = reactive({
  ...
  departments: [requiredEspecialSelectValidator],
})
...
const getCorporationAll  = async () => {
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      corporationItems.value.filter(e => e.id >= corporationMin.value) 
    }
  } catch (error) {
        ...
        <VRow>
          ...
          <VCol cols="12" sm="6" md="6">
            <VAutocomplete 
              ...
              @update:model-value="$event => generalfilter.setGroupingPolitical( $event, false )" 
            />
          </VCol>
        </VRow>
      </VCardText>
