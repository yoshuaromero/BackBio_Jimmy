...
const handlerNext = async () => {
  ...
  axios.get(`/v1/candidates/candidate-arming-file-generation`, {
    params: payload,
    responseType: "blob", // importante
  }).then(({ data }) => {
    //console.log(data)
    const nameFile = 'Generación Archivo de Armado de Candidatos.xlsx'
    ...
    if (error.response && error.response.status < 500) {
      ...
      Swal.fire({ icon: "warning", text: 'No hay registros para generar el archivo' })
    }
...
<template>
  <VCard :title="title">
    <VForm ref="form" validate-on="submit" @submit.prevent="handlerNext">
      <VCardText>
        <VRow>
          <h4>{{ $t('candidate_generationList_report.title') }}</h4>
        </VRow>
        <br>
        <VRow class="mb-2">
          <VCol cols="12" md="6">
