...
const getData = async payload => {
  try {
    const { data } = await axios.get('/v1/inscriptions/approve', { params: payload.value })
...
const handlerClickInsription = payload => {
  router.push({ name: "preinscription-approved-id", params: { id: payload.id } })
}
</script>
...
<route lang="yaml">
name: "preinscription-approved-list"
meta:
  authRequired: true
</route>
