...
const fields = ref([
  ...
  { key: 'birthdate', label: t('module_reports.generalDetailedCandidates.birthdate') }, 
  { key: 'age', label: t('module_reports.generalDetailedCandidates.age') }, 
  { key: 'email', label: t('module_reports.generalDetailedCandidates.email') }, 
  { key: 'address', label: t('module_reports.generalDetailedCandidates.address') }, 
  { key: 'phone', label: t('module_reports.generalDetailedCandidates.phone') }, 
  { key: 'statusCandidate', label: t('module_reports.generalDetailedCandidates.statusCandidate') }, 
  { key: 'acceptanceMethod', label: t('module_reports.generalDetailedCandidates.acceptanceMethod') }, 
  ...
  { key: 'inscriptionStatus', label: t('module_reports.generalDetailedCandidates.inscriptionStatus') },
  ...
  { key: 'renuncia', label: t('module_reports.generalDetailedCandidates.renuncia') },
  
  { key: 'revocatoria', label: t('module_reports.generalDetailedCandidates.revocatoria') },
  
  { key: 'reject', label: t('module_reports.generalDetailedCandidates.reject') },
  { key: 'dateE6', label: t('module_reports.generalDetailedCandidates.dateE6') },
  { key: 'dateE7', label: t('module_reports.generalDetailedCandidates.dateE7') },
  { key: 'dataE8', label: t('module_reports.generalDetailedCandidates.dataE8') },
  { key: 'managerDni', label: t('module_reports.generalDetailedCandidates.managerDni') },
  
  { key: 'managerNameFull', label: t('module_reports.generalDetailedCandidates.managerNameFull') },
  
  { key: 'managersurnameFull', label: t('module_reports.generalDetailedCandidates.managersurnameFull') },
  

  { key: 'celularPhoneManager', label: t('module_reports.generalDetailedCandidates.celularPhoneManager') },
  { key: 'emailManager', label: t('module_reports.generalDetailedCandidates.emailManager') },
...
const exportar = type => {
  ...
  axios.get(`/v1/candidates/report-export-genaral-datail`,{
    params: payload,
    responseType: "blob", // importante
  }).then(({ data }) => {
    const nameFile = type === 'XLS' ? 'Reporte General y Detallado de Candidatos  .xlsx' : 'Reporte General y Detallado de Candidatos.csv' 
...
<template>
  <div>
    <VBreadcrumbs :items="[ $t('common.home'), $t('module_reports.title'), $t('module_reports.generalDetailedCandidates.title3') ]" />
    ...
    <VCard v-if="showItems">
      ...
      <DataTableServerSide
        ref="datatable"
        endpoint="/v1/candidates/candidate-report-general-datail"
        :params="filters"
        :fields="fields"
        :loading="isLoading"
      >
        ...
        <template #cell(managerNameFull)="{ item }">
          <div class="d-flex justify-center">
            {{ item.managerFirstname +"  "+item.managerSecondName }}  
          </div>
        </template>
        <template #cell(managersurnameFull)="{ item }">
          <div class="d-flex justify-center">
            {{ item.managerFirstsurname +"  "+item.managerSecondSurname }}  
          </div>
        </template>
...
<route lang="yaml">
  name: "reports-candidate-generalDetailReporte"
  meta:
  authRequired: true
  </route>
