...
const getData = async payload => {
  try {
    const { data } = await axios.get('/v1/inscription-change/extemporaneous/approve', { params: payload.value })
...
const { isLoading, data } = useQuery(
  ['inscription-modifies-extemporaneous-approved', payload], 
  () => getData(payload), {
    cacheTime: 1000 * 60 * 5,
...
const handlerClickInsription = payload => {
  router.push({ name: "inscription-modifies-approved-id", params: { id: payload.id } })
}
</script>
...
<route lang="yaml">
name: "inscription-modifies-extemporaneous-approved-list"
meta:
  authRequired: true
</route>
