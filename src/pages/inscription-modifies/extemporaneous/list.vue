...
const disableCreateInscription = ref(true)
...
const getData = async payload => {
  try {
    const { data } = await axios.get('/v1/inscription-change/extemporaneous/not-send', { params: payload.value })
...
const { isLoading, data, refetch } = useQuery(
  ['inscriptions-inprogress-extemporaneous-pending', payload], 
  () => getData(payload), {
    cacheTime: 1000 * 60 * 5,
...
const handlerClickInsription = payload => {
  router.push({ name: "inscription-modifies-id", params: { id: payload.id } })
}

onMounted(() => {
  if(!can('create', 'inscription-change')){
    disableCreateInscription.value = false
  }
  refetch()
})
</script>
...
<template>
  <div>
    ...
    <VRow class="my-3 mx-1">
      <VSpacer />
      <VBtn v-if="disableCreateInscription" color="secondary" :to="{ name: 'inscription-modifies-create' }">
        {{ t('common.btn_create_request') }}
      </VBtn>
    </VRow>
...
<route lang="yaml">
name: "inscription-modifies-extemporaneous-list"
meta:
  authRequired: true
</route>
