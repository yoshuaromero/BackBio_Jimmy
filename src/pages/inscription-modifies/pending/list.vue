...
const fields = ref([
  ...
  { key: "stateDescriptionE7", label: t('formsPendingApproval.state').toUpperCase() },  
  { key: "inscriptionTypeName", label: t('formsPendingApproval.breadcrumbs.inscriptionTypeName').toUpperCase() },
  { key: 'DocumentsE7_charged', label: t('formsPendingApproval.DocumentsE7_charged').toUpperCase() },
  { key: "approvedForm_E7", label: t('formsPendingApproval.approvedForm_E7').toUpperCase() },
...
const getDataforPdf = async () => {
  const requestBody = {
    "grouping_political_type_id": -1,
    "grouping_political_id": -1,
...
const exportFileXls = async value => {
  const requestBody = {
    "grouping_political_type_id": -1,
    "grouping_political_id": -1,
    "inscription_type_id": -1,
...
const GoSupportInscription = id => {
  ...
  router.push({ name: 'inscription-modifies-pending-id', params: { id: Number(id), tab: 'inscription-docsupports' } })
}
    
const GoViewApprovallE7 = (id, name) => {
  // pantalla de aprobación
  router.push({ name: 'inscription-modifies-pending-id', params: { id: Number(id), tab: 'summary' } })
}

// data para traer la fase actual
...
<template>
  <div>
    <VCard class="mb-6">
      ...
      <FiltersFormsPendingApproval @next="getInscriptions" />
    </VCard>
    <VCard v-if="items.length == 0">
        ...
        <div class="me-3 mt-3 mb-2 text-right">
          <VMenu open-on-hover class="me-3 mb-2 ms-3 mt-3">
            <template #activator="{ props }">
              <VBtn v-bind="props" prepend-icon="tabler-table-export">
                ...
                <ul class="pa-3">
                  ...
                  <li class="my-1">
                    <a
                      class="dropdown-item"
                      href="javascript:void(0)"
                      @click.prevent="exportFileXls('xlsx')"
                    >
      ...
      <div>
        <DataTableServerSideWithLogo
          ref="datatable"
          endpoint="/v1/inscription-change/pending"
          :params="filters"
          :fields="fields"
        >
          ...
          <template #cell(DocumentsE7_charged)="{ item }">
            <VBtn variant="plain" @click.prevent="GoSupportInscription(item.e7Id)">
          ...
          <template #cell(approvedForm_E7)="{ item }">
            <VBtn variant="plain" @click.prevent="GoViewApprovallE7(item.e7Id)"> 
              {{ t('formsPendingApproval.breadcrumbs.bntDocumentsApprovedE7') }}
            </VBtn>
          </template>
        </DataTableServerSideWithLogo>
      </div>
...
<route lang="yaml">
name: "inscription-modifies-pending-list"
meta:
  authRequired: true
</route>
