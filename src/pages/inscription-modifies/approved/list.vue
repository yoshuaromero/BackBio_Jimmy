...
const getData = async payload => {
  try {
    const { data } = await axios.get('/v1/inscription-change/approve', { params: payload.value })
...
const { isLoading, data } = useQuery(
  ['inscription-modifies-approved', payload], 
  () => getData(payload), {
    cacheTime: 1000 * 60 * 5,
...
<route lang="yaml">
name: "inscription-modifies-approved-list"
meta:
  authRequired: true
</route>
