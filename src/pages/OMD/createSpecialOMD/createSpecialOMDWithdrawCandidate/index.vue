...
const handlerSubmit = async () =>{
  ...
  if (valueSelect.value == -1 && disabledCheck.value ) {
    ...
    Swal.fire({ icon: 'warning', text: 'Porfavor seleciona el Reglón o el Check para Retirar la Lista completa' })
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios.post(`/v1/omd/insert/omd/special/`,formData, {
          ...
          //   downloadBlob(data, "Crear OMD Especial (Retirar Candidato)",'application/pdf')
    ...
    <VCard>
      <VForm ref="form" validate-on="submit" @submit.prevent="handlerSubmit">
        <VCardText>
          ...
          <VListItemTitle class="demo-space-x  ps-10 mt-3 mb-2">
            Retirar Lista Completa de Candidatos:
          </VListItemTitle>
          <hr>
          <VRow class="mt-3">
            <VCol cols="12" md="6">
              <VCheckbox
                ...
                :true-value="1"
                :false-value="0"
                label="Check para retirar la lista completa"
